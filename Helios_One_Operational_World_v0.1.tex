...
{\Large One governed operating environment for understanding conditions, coordinating response, supervising autonomy, and proving outcomes.}\vspace{5mm}
\begin{columns}[T]
\column{.32\textwidth}
\begin{block}{Operational world}
\small Digital Twin, Sites, zones, routes, cameras, sensors, robotic assets, human teams, evidence, and current readiness.
\end{block}
\column{.32\textwidth}
\begin{block}{Governed action}
\small Attention, Incidents, Missions, resources, Commands, intervention, observed effects, reconciliation, and Records.
\end{block}
\column{.32\textwidth}
\begin{block}{Cortex intelligence}
\small Assessment, explanation, recommendations, policy-bounded intent, memory, learning candidates, and Agent coordination.
\end{block}
\end{columns}
\vspace{4mm}
...
\end{block}
\vfill
\tagcurrent\ Operator, spatial, execution, media, Cortex, and authority foundations \quad
\tagdev\ Canonical Mission and Command integration \quad
\tagplanned\ Policy autonomy and multi-Agent coordination
\end{frame}
...
\begin{itemize}
  ...
  \item A ground inspection robot is staged nearby.
\end{itemize}
\end{block}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[text,font=\scriptsize] at (1.55,1.35){Operations};
  \fill[active] (3,.7) rectangle (5.25,2.1);
  \node[text,font=\scriptsize] at (4.13,1.4){Transformer yard};
...
\small Cortex correlates evidence, explains uncertainty, compares options, and recommends an aerial observation Mission. The operator accepts, modifies, rejects, or asks why.\vspace{2mm}\par
{\color{amber}\bfseries Human role: decide.}
\end{block}
\column{.32\textwidth}
\begin{block}{PHASE 3 -- POLICY AUTONOMY}
\small Evidence and policy qualify. Cortex initiates the approved observation Mission through normal readiness, resource, and Command admission. The operator supervises.\vspace{2mm}\par
{\color{blue}\bfseries Human role: supervise.}
\end{block}
\end{columns}
\vspace{5mm}
\begin{block}{PHASE 4 -- MULTI-AGENT COORDINATION}
\centering Site, Asset, Mission, and Operations Agents request and broker capabilities. Humans govern priorities, exceptions, policy, and consequence. No Agent gains authority from another Agent message alone.
\end{block}
\end{frame}
...
\scriptsize Tasks: overhead view $\rightarrow$ track condition $\rightarrow$ inspect blind area $\rightarrow$ preserve evidence \hfill HOLD \quad REDIRECT \quad RETURN \quad ABORT\par\vspace{1mm}
...
\begin{itemize}
  ...
  \item Buildings, roads, zones, routes, POIs, docks, and recovery.
  \item Cameras, sensors, coverage, and freshness.
  \item Asset deployments, capability, and readiness.
  \item Missions, policy applicability, evidence, and revision.
\end{itemize}
\end{block}
\begin{block}{Cortex uses structured context}
\small Location, reachability, proximity, coverage, restrictions, resource qualification, route consequence, and evidence relationships.
\end{block}
\column{.51\textwidth}
\begin{block}{One accountable Mission picture}
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[draw=line,fill=active,text=text,rounded corners,minimum width=2.6cm,minimum height=.78cm] (d) at (3,4){Detection};
  \node[draw=blue,fill=panel,text=text,rounded corners,minimum width=2.6cm,minimum height=.78cm] (a) at (3,3){Aerial observation};
  \node[draw=mint,fill=panel,text=text,rounded corners,minimum width=2.6cm,minimum height=.78cm] (g) at (3,2){Ground inspection};
  \node[draw=green,fill=panel,text=text,rounded corners,minimum width=2.6cm,minimum height=.78cm] (h) at (3,1){Human verification};
  \draw[-{Latex},blue,line width=1.2pt] (d)--(a);
  \draw[-{Latex},mint,line width=1.2pt] (a)--(g);
  \draw[-{Latex},green,line width=1.2pt] (g)--(h);
  \node[text,font=\scriptsize,align=left] at (6.4,2.5){Same Site context\par Separate accountable Missions\par Shared Incident and evidence\par Human and machine roles remain distinct};
\end{tikzpicture}
\end{block}
\begin{block}{Trust is visible}
\centering\small Authorized $\rightarrow$ Delivered $\rightarrow$ Accepted $\rightarrow$ Effect observed $\rightarrow$ Outcome known
\end{block}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[draw=green,fill=panel,text=text,rounded corners,minimum width=3.2cm,minimum height=1.15cm,align=center] (n) at (2,4.3){North River\par Ready -- 2 Missions};
  \node[draw=amber,fill=panel,text=text,rounded corners,minimum width=3.2cm,minimum height=1.15cm,align=center] (e) at (2,2.5){East Switching\par Degraded -- support needed};
  \node[draw=blue,fill=panel,text=text,rounded corners,minimum width=3.2cm,minimum height=1.15cm,align=center] (h) at (2,.7){Hillcrest Hydro\par Local operation};
  \node[draw=mint,fill=active,text=text,rounded corners,minimum width=4.6cm,minimum height=1.45cm,align=center] (o) at (7.5,2.5){Enterprise Operations + Cortex\par posture, Attention, policy, coordination};
  \node[draw=blue,fill=panel,text=text,rounded corners,minimum width=3.8cm,minimum height=1.2cm,align=center] (r) at (12.8,2.5){Cross-Site request\par aerial thermal capability};
  \draw[-{Latex},line,line width=1.2pt] (n)--(o);
  \draw[-{Latex},line,line width=1.2pt] (e)--(o);
  \draw[-{Latex},line,line width=1.2pt] (h)--(o);
  \draw[-{Latex},blue,line width=1.4pt] (o)--(r);
\end{tikzpicture}
\vspace{4mm}
\begin{block}{The payoff}
\centering{\LARGE\bfseries Your systems. Your Sites. One coherent operating experience.}\par\vspace{2mm}
{\small See the operation. Coordinate the response. Prove the outcome.}
\end{block}
\vfill
\tagcurrent\ Strong operator, spatial, execution, media, and Cortex foundations \quad
\tagdev\ Canonical integration \quad
\tagplanned\ Policy autonomy, resilient Site cells, and multi-Agent coordination
\end{frame}
